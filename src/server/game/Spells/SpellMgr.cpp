...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
		...
		case 18754: // Improved succubus - problems with apply if target is pet
            ...
            spellInfo->EffectBasePoints[0] = -330;           // reduce cast time of seduction by 22% 
            spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
            count++;
            break;
        case 18755:
            spellInfo->EffectApplyAuraName[0] = SPELL_AURA_ADD_FLAT_MODIFIER;
            spellInfo->EffectBasePoints[0] = -660;           //  reduce cast time of seduction by 44%
            spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
            count++;
            break;
        case 18756:
            spellInfo->EffectApplyAuraName[0] = SPELL_AURA_ADD_FLAT_MODIFIER;
            spellInfo->EffectBasePoints[0] = -990;           //  reduce cast time of seduction by 66%
            spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
            count++;
            break;
